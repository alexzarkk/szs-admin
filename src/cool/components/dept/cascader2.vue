	...
	methods: {
		...
		setChecked(val){
			...
			if (!val) {
				...
				const _ = (id)=>{
					let p = this.dept[id]
					if(p) {
						this.checked.unshift(p.id+'')
						if(p.pid&&(p.pid!=330000 || this.$store.getters.userInfo.departmentId==330000)) _(p.pid)
					}
				}
				_(val)
		...
		refresh() {
			this.$service.system.dept.list({load:true}).then((res) => {
					...
					if(this.id) this.setChecked(this.id)
				}).catch((err) => {
