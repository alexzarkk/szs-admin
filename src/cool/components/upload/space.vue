...
export default {
	...
	methods: {
		...
		chooseVideo() {
			uni.chooseVideo({
				success: (res) => {
					if(this.zz.math(res.size/1024/1024,0)>100) {
						return this.$message.error('文件不得大于100m（请压缩后上传）')
					}
					this.file.loading.uploadVideo = true;
					
					
					// return console.log(res.tempFile.file());
		...
		// 编辑分类
		editCategory(item = {}) {
			if(this.category.list.length>=12) {
				return this.$message.error('最多只能设定12个分类！')
			}
		...
		// 确认选中文件
		confirmFile() {
			const urls = this.selection.filter((e, i) => i < this.limit)
				.map((e) => e.url)
				.join(",");
