...
export default {
	...
	methods: {
		...
		registerRest(instance) {
			...
			$tdtComponent && $tdtComponent.getType && $tdtComponent.getType() !== 3 && $tdtMap.addOverLay(
			$tdtComponent); //添加覆盖物（除信息窗口）
			this.addEvents();
			this.setProps();
			this.addWatchers();
			if ($tdtComponent) {
			...
			if (enableProps[tag]) {
				enableProps[tag].forEach(prop => {
					if (this[prop] !== undefined) {
						this[prop] ? $tdtComponent[`enable${capitalize(prop)}`]() : $tdtComponent[
							`disable${capitalize(prop)}`]();
					}
				});
			}
			//组件的可设置属性
		...
		addWatchers() {
			...
			if (enableProps[tag]) {
				enableProps[tag].forEach(prop => {
					this.$watch(prop, val => {
						val ? $tdtComponent[`enable${capitalize(prop)}`]() : $tdtComponent[
							`disable${capitalize(prop)}`]();
					});
				});
			}
