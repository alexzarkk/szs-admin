			...
			<div class="box">
				...
				<el-form ref="form" class="form" size="medium" :disabled="saving">
					...
					<el-form-item label="密码">
						<el-input type="password" placeholder="请输入密码" v-model="form.password" maxlength="20" auto-complete="off"></el-input>
					</el-form-item>
					<el-alert v-if="tip" title="若忘记密码,请使用APP找回" type="warning" effect="dark"></el-alert>
					
					<el-form-item label="验证码" class="captcha">
						<el-input placeholder="请输入图片验证码" maxlength="4" v-model="form.verifyCode" auto-complete="off" @keyup.enter.native="next"></el-input>

						<captcha ref="captcha" class="value" v-model="form.captchaId" @change="captchaChange"></captcha>
					</el-form-item>
				</el-form>
				<el-button round size="mini" class="submit-btn" @click="next" :loading="saving">登录</el-button>
			
				<block v-if="qrcode">
					<!-- <view class="flex justify-between">
						<div class="padding-top flex  justify-center">
		...
		async next() {
			...
			try {
				...
				if(err=='密码不正确~') {
					this.tip = true
					let _=this
					setTimeout(function() {_.tip=false}, 5000);
				}
			}
		},
		help(){
