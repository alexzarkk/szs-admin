<template>
	<cl-layout>
		<el-row>
			<el-col>
				<el-button size="mini" @click="updateImg('poi', 20)">poi</el-button>
				<el-button size="mini" @click="updateImg('trail', 20)">trail</el-button>
...
export default {
	...
	methods: {
		...
		async saveImg(url, pre = 'zts'){
			...
			try{
				const newUrl = await uploadFile({ filePath: url, cloudPath })
				this.src = newUrl
				return newUrl
			}catch(e){
